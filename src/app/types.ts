...
export type Hotel   = {
    ...
    cityId: number; cityName: string; countryId: string; countryName: string;
};
...
export type ErrorResponse = {
    code: number; error: true; message: string; waitUntil?: string;
};

export type StartSearchResponse = { token: string; waitUntil: string };
export type GetSearchPricesResponse = { prices: PricesMap };
export type StopSearchResponse = { message: string } | { status: "cancelled"; message: string };
